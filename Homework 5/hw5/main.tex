...
$$\boldsymbol{\beta}=
    ...
    \begin{enumerate}
        ...
        \item $\blackcircle$ Append a value of 1 to be the first entry of $\zv$
...
Plot the average training cross-entropy on the y-axis vs the number of epochs on the x-axis for the mentioned learning rates. In the \textbf{same figure}, plot the average \ntset\thinspace cross-entropy loss. You may make a separate figure for each learning rate.
 ...
 \begin{tcolorbox}[fit,height=18cm, width=15cm, blank, borderline={1pt}{-2pt}]
    ...
    \includegraphics[scale=0.4]{img/plot2.png} Learning Rate = 0.001
    \end{tcolorbox}
...
Examine and comment on the the plots of training and \ntset\thinspace cross-entropy. How does adjusting the learning rate affect the convergence of cross-entropy of each dataset?
 ...
 \begin{tcolorbox}[fit,height=10cm, width=15cm, blank, borderline={1pt}{-2pt}]
    %solution 
    \begin{itemize}
    \item Learning Rate = 0.1: Training cross-entropy decreases quickly and reaches close to zero but testing cross-entropy decreases for a very small interval and then increases. The model performs poorly with such a large learning rate.
    \item Learning Rate = 0.01: Training cross-entropy decreases smoothly, whereas testing cross-entropy first decreases and then gradually increases. It outperforms the previous used learning rate but still does not perform well when we increase the number of hidden units.
    \item Learning Rate = 0.01: The values of training and testing cross-entropies are very close during all epochs, which means our model generalizes well to held out data. However we observe that the overall accuracy is worse than using a learning rate of 0.01, since the loss value is higher than before.
    \end{itemize}
    In general, the model fits better the training data if we increase the learning rate, however it overfits, as we see clearly in the first figure for a 0.1 learning rate. Decreasing the learning rate improves the performance of our model, but we also need to keep in mind that if we keep increasing the learning rate, our model can become very slow.
    \end{tcolorbox}
