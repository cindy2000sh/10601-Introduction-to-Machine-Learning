...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item $I(X;Y)$ 
    \end{enumerate}
    \begin{tcolorbox}[fit,height=8cm, width=\linewidth, blank, borderline={1pt}{-2pt}]
    \includegraphics[width=0.99\linewidth]{figs/screen1.png}
    \end{tcolorbox}
    
    
    \item[5] [2 points] Prove the symmetry of mutual information between any two random variables X,Y, i.e., $I(X;Y) = I(Y;X)$.\\
    \begin{tcolorbox}[fit,height=12cm, width=\linewidth, blank, borderline={1pt}{-2pt}]
    \includegraphics[width=0.99\linewidth]{figs/screen2.png}
    \end{tcolorbox}
...
\begin{tcolorbox}[fit,height=13cm, width=\linewidth, blank, borderline={1pt}{-2pt},nobeforeafter]
%solution
\includegraphics[width=0.99\linewidth]{figs/screen3.png}
\end{tcolorbox}
...
\begin{tcolorbox}[fit,height=3cm, width=\linewidth, blank, borderline={1pt}{-2pt},nobeforeafter]
%solution
No, new state and action always depend on the current state, so we cannot make the iid assumption. As mentioned during lecture, Experience Replay is a method to tackle this issue, storing some samples and then randomly picking a stored sample when updating. Another similar trick is Prioritized Experience Replay, with the difference that sampling prioritizes experiences with high error.
\end{tcolorbox}
...
%solution
\includegraphics[scale=0.45]{figs/raw.png}
\includegraphics[scale=0.45]{figs/tile.png} \\
We observe that when using the raw features, we keep getting -200 as our total and rolling mean reward until 1000 episodes and then we get lower values but still the rolling mean revolves around low values (approximately $\leq 180$). On the other hand, when using the tile features, we only get -200 total and rolling mean reward only until the first episodes. After that, our rewards still fluctuate but on larger values. We can see that the rolling mean increases almost linearly and reaches a much higher value than the raw feature rolling mean. It is evident that the training with the raw features takes many more episodes to start converging, whereas the training with the tile features converges at a far lower number of episodes. This was expected, since the tile features are more complex than the simplistic raw features.
\end{tcolorbox}
...
%solution
We observe that the first figure corresponds to the tile features, while the second one to the raw features. That is because, for raw features, the value function is linear, and hence we expect linear decision boundaries (right figure), whereas for tile features, the value function is non-linear, so we expect complicated non-linear decision boundaries as the ones depicted in the first figure.\\
The decision boundaries of the raw features is straight-forward: When the car's velocity is higher than a certain threshold, the car will always choose to go right (action 2). While before a certain position threshold (around 0), the car will go left for a velocity lower than -0.02, it will stay in place for values greater than this threshold.\\
The decision boundaries for the tile features are more complicated: There is a trend for going right for positive velocities, while for negative velocities, the car usually goes left. There are also some small areas where the car deviates from the described trend, for example for very low velocities and positions, or for large positions and low velocities.
\end{tcolorbox}
