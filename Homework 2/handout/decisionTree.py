...
def preorder(root):
	if root:
		...
		if root.max_ig_ind != None:
			print(root.depth*symb,root.val[root.max_ig_ind][0],'=',root.attr,': [',root.label[0],lab1,'/',root.label[1],lab2,']')
		preorder(root.left)
		preorder(root.right)
...
def train(root, max_depth):
	...
	if max_info_gain < 0: #check this condition
		return
	if root.depth == 0:
		na = 0
		nb = 0
		...
		if na >= nb:
			...
			root.pred = lab2
	n1 = n2 = n3 = n4 = 0
	# root.left.ref1 = val1[max_info_gain]
	# root.right.ref2 = val2[max_info_gain]
	root.left = Node(splitData(root.val, max_info_gain, val1[max_info_gain]), root.depth + 1)
	root.right = Node(splitData(root.val, max_info_gain, val2[max_info_gain]), root.depth + 1)
	root.left.attr = val1[max_info_gain]
	root.right.attr = val2[max_info_gain]
...
def traverse(root, dataset, ind, r):
	if root.left and dataset[root.left.max_ig_ind][ind] == val1[root.left.max_ig_ind]:
		traverse(root.left, dataset, ind, r)
	elif root.right and dataset[root.right.max_ig_ind][ind] == val2[root.right.max_ig_ind]:
...
def predict(dataset, root, r):
	for i in range(1, len(dataset[0])): 
		if dataset[root.left.max_ig_ind][i] == val1[root.left.max_ig_ind]:
...
#set reference attribute values

val1 = []
val2 = ['empty' for i in range(len(data) - 1)]
done = True
for i in range(1, len(data[0])):
	for j in range(len(data) - 1):
		if val2[j] == 'empty':
...
if max_depth == -1:
	...
	print('[',numb_of_label_1,lab1,'/',numb_of_label_2,lab2,']')
else:
	root = Node(data, 0)
...
# tr_error = np.array([0.4429, 0.2013, 0.1342, 0.1141, 0.1074, 0.0872, 0.0738, 0.0671])
# tst_error = np.array([0.5060, 0.2168, 0.1567, 0.1687, 0.1807, 0.1687, 0.1928, 0.2048])
# dpth = np.array([0, 1, 2, 3, 4, 5, 6, 7])

# plt.plot(dpth, tr_error, 'r')
# plt.plot(dpth, tst_error, 'b')
# plt.show()